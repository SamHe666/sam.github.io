...
事实上，理解了PLSA模型，也就差不多快理解了LDA模型，因为LDA就是在PLSA的基础上加层贝叶斯框架。LDA其实就是在PLSA的基础上给这两参数，$p(z|d)$ 和 $p(w|z)$），加了两个先验分布：一个主题分布的先验分布$Dir(\alpha_{topics})$和一个词语分布的先验分布$Dir(\alpha_{words})$。通常而言，LDA 比 PLSA 效果更好，因为它可以轻而易举地泛化到新文档中去。在 PLSA 中，文档概率是数据集中的一个固定点。如果没有看到那个文件，我们就没有那个数据点。然而，在 LDA 中，数据集作为训练数据用于文档-主题分布的狄利克雷分布。即使没有看到某个文件，我们可以很容易地从狄利克雷分布中抽样得来，并继续接下来的操作。
